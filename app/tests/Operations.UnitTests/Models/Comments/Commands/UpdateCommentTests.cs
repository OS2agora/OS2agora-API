...
namespace Agora.Operations.UnitTests.Models.Comments.Commands
{
    public class UpdateCommentTests : ModelsTestBase<UpdateCommentCommand, Comment>
    {
        ...
        public async Task UpdateCommentCommand_HasRole(bool respondedToHearing, bool ownsComment)
        {
            var request = new UpdateCommentCommand();
            SecurityExpressionsMock.Setup(x => x.IsHearingResponder(It.IsAny<int>()))
                .Returns(respondedToHearing);
            SecurityExpressionsMock.Setup(x => x.IsCommentOwnerByCommentId(It.IsAny<int>()))
                .Returns(ownsComment);


            if (!respondedToHearing || !ownsComment)
            {
                FluentActions
                    .Invoking(() =>
        ...
        [TestCase(false)]
        [TestCase(true)]
        public async Task UpdateCommentStatusCommand_HasRole(bool isHearingOwner)
        {
            var request = new UpdateCommentCommand();
